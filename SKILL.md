...
这是一个**资料获取与事实归档**工具。输入甲方需求文档或一个主题词,默认交付一份**可用 XMind 打开的超详细资料型思维导图**(`.xmind`)。
...
3. `fetch_sources.py` 抓取公开来源文本。
4. `evidence_cards.py` 校验证据卡。
5. `coverage_audit.py` 审计必查核心是否有证据。
6. `research_loop.py` 根据覆盖缺口生成二轮/三轮深搜补搜计划。
7. 用模型解释层把证据讲清楚,补足概念解释、层级拆分和报告衔接;新增事实必须回到补搜与证据卡。
8. 用 `outline_from_evidence.py` 生成或辅助生成 `06_提炼笔记/调研大纲.md`。
9. `outline_audit.py` 审计大纲。
10. `md_to_xmind.py` 生成 `.xmind`。
11. `xmind_audit.py` 审计最终 `.xmind`。
12. 完整交付模式下,用 `report_from_evidence.py` 生成报告 Markdown,用 `report_audit.py` 审计,再用 `report_to_docx.py` 生成 `.docx`。

**没有证据卡和覆盖审计,不得写正式大纲。大纲审计失败,不得生成最终 XMind。**

## 关键原则

- **默认只交 XMind**:最终回复只给 `.xmind` 绝对路径和简短验证结果。
- **完整交付可交 Word**:只有用户明确要求报告、Word、docx 或完整交付时,才追加 `.docx`。Word 是资料报告,不是策展方案。
- **只做资料,不做策展判断**:只写是什么、何时、何地、谁、数据多少、来源在哪、是否有争议。
- **证据优先**:导图节点必须来自用户资料、公开来源或明确的未找到记录,不能来自模板填空。
- **允许模型解释,不允许模型造事实**:大模型可以把证据讲清楚、拆层级、写衔接、解释概念机制;但时间、人物、地点、数据、政策、来源和最新动态必须由证据卡支撑。模型提出的新增事实只能进入补搜任务或待核实清单。
...
- **Word 必须是连续报告**:Word 不能是 XMind 节点拷贝,也不能是证据卡列表。它要有章节、导言、连续段落、来源口径和待核实问题。
...
- 可以补:概念定义的小白解释、分类之间的差异、工具工作方式、行为流程、段落衔接、章节标题、报告文气。
...
| `fetch_sources.py` | 抓取公开网页并抽取基础文本 |
