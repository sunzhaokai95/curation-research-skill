...
6. 用 `outline_from_evidence.py` 生成或辅助生成 `06_提炼笔记/调研大纲.md`。
7. `outline_audit.py` 审计大纲。
8. `md_to_xmind.py` 生成 `.xmind`。
9. `xmind_audit.py` 审计最终 `.xmind`。
10. 完整交付模式下,用 `report_from_evidence.py` 生成报告 Markdown,用 `report_audit.py` 审计,再用 `report_to_docx.py` 生成 `.docx`。
...
只要有必查核心证据不足,就回到搜索与证据抽取。不要用“需要补”占位。
